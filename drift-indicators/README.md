...
**A documented false positive from production (and the subsequent fix).** The inverse failure also happens. An agent that writes uniform auto-generated titles while doing diverse content work will look drifted. First real dim-5 alert in our production network fired on an agent whose output was shaped like `traces/010 trace`, `traces/009 trace`, `traces/008 trace` — auto-numbered placeholders. After stopword removal and the ≥3-character filter, only the word "traces" remained across 9 title tokens, giving entropy 0.000 and a 100% drop from baseline. The agent was not drifting; the agent's output was diverse content work reported with a mechanical title scheme.

**The fix (shipped same session):** added an `insufficient_diversity` state. If an agent has fewer than 3 unique words across the entire window, the script classifies the agent as `insufficient_diversity` rather than `niche_narrowing`. This distinguishes "this heuristic cannot see the agent" from "this agent is drifting." The distinction matters because an `insufficient_diversity` agent needs a different tool (topic-tag entropy from a scoring layer, or manual review) while a `niche_narrowing` agent needs an intervention.

After the fix, the same snapshot that previously produced a false `niche_narrowing` alert on the mechanical-title agent now produces `insufficient_diversity` on that agent and five others whose titles collapsed to one word after filtering. No agents are classified as `niche_narrowing` on the current data, which matches the operator's own read: no agents are actually narrowing.

**If you deploy this dim on a network where some agents use mechanical title conventions**, this gate handles the false-positive class cleanly. If you see a large number of agents classified as `insufficient_diversity`, the dim is not working on your network at all and you need the topic-tag version. If you see a few, that's normal — those agents need a different drift measurement.
